...
import { Callout, Card, Note, Section, Stat } from '../components/ui';
import {
  compute,
  sensitivity,
  simulate,
...
export function ScenarioCompare() {
  ...
  const rows = useMemo(() => {
    const out: { name: string; r: ComputeResult; data: Pursuit }[] = [];
    for (const p of pursuits) {
      try {
        out.push({ name: p.data.name, r: compute(p.data), data: p.data });
      } catch {
  ...
  const metric = (label: string, fn: (r: ComputeResult, d: Pursuit) => string | number) => (
    <tr>
      <td>{label}</td>
      {rows.map((x, i) => (
  ...
  return (
    <Section
      ...
      sub="Every saved pursuit side by side. Duplicate a pursuit (top bar), change one assumption, and read the deltas here without losing the original."
    >
      <div className="card flush">
        <div className="ch">
          <h3>Side-by-Side</h3>
        ...
        <div className="tablewrap">
          <table>
            ...
            <tbody>
              {metric('Total price', (r) => money0(r.total))}
              {metric('Phase 1', (r) => money0(r.phase1Price))}
              {metric('Phase 2', (r) => money0(r.phase2Price))}
              {metric('Cost P50', (r) => money0(r.costP50))}
              {metric('Cost P80', (r) => money0(r.costP80))}
